...
test('Google test using playwright', async () => {
    ...
    for (let index = 0; index < 3; index++) {
        ...
        for (const keyword of keywords) {
            ...
            await page.goto('https://www.google.com/search?q='+lowercasedWord+'test');
            await page.waitForTimeout(2000);
            await expect(page.getByRole('link', { name: keyword+'   ' }).first()).toBeEnabled();
            await page.getByRole('link', { name: keyword+'   ' }).first().click();
